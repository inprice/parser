...
public class Amazon_NL_Test {
	...
	private final Website site = new AmazonNL();
	...
	public void test_product_1() {
		...
		assertEquals(
		    "The 80/20 Principle: The Secret of Achieving More with Less: Updated 20th anniversary edition of the productivity and business classic (English Edition)",
		    link.getName());
	...
	public void test_product_2() {
		...
		assertEquals(
		    "Platform Revolution: How Networked Markets Are Transforming the Economyand How to Make Them Work for You (English Edition)",
		    link.getName());
		assertEquals("10.88", link.getPrice().toString());
		assertEquals(
		    "Geoffrey G. Parker (auteur), Marshall W. Van Alstyne (auteur), Sangeet Paul Choudary (auteur) & 0 meer",
		    link.getBrand());
		assertEquals("Amazon", link.getSeller());
		assertEquals("Laat het op je Kindle of een ander apparaat leveren", link.getShipment());
		assertTrue(link.getSpecList().size() > 0);
