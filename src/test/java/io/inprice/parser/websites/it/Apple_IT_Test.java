...
public class Apple_IT_Test {
	...
	private final Website site = new AppleIT();
	...
	public void test_product_3() {
		...
		assertEquals(LinkStatus.AVAILABLE, link.getStatus());
		assertEquals(sku, link.getSku());
		assertEquals("Apple Watch Series 4 GPS, cassa 40 mm in alluminio color argento e cinturino Sport bianco",
		    link.getName());
	...
	private void setMocks(String sku) {
		final String SHIPMENT_DATA = String.format("{" + "'head': {" + "  'status': '200'," + "  'data': {}" + "},"
		    ...
		    + "          }" + "        ]," + "        'promoMessage': 'Free Shipping'," + "        'deliveryOptionsLink': {"
		    + "          'text': 'Get delivery dates'," + "          'dataVar': {}," + "          'newTab': false"
		    + "        }," + "        'showDeliveryOptionsLink': true," + "        'messageType': 'Ship',"
		    + "        'basePartNumber': 'MTXP2'," + "        'commitCodeId': '0',"
